...
for file in $proto_files; do
  ...
  if grep -q "service " "$file"; then
    ...
    api_component=$(echo "$file" | awk -F'/' '{ print $3 }')
    ...
    destination_file="$api_type-$api_version-$api_component-$proto_file_name.yaml"

cat << EOF > "$CATALOG_INFO_DIR_RELATIVE/$destination_file"
apiVersion: backstage.io/v1alpha1
kind: API
metadata:
  name: $api_component-$api_type-$proto_file_name
  namespace: core
